# Manual server setup and developer reference

These instructions are for developers and users connecting another MCP client or managing the server themselves. The packaged Multicam plugin starts its bundled runtime automatically; plugin users do not need these commands. See the [plugin setup](../README.md#get-started-in-codex) first.

## Run one shared server

For a manually managed installation, use one long-lived [Streamable HTTP](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) server. Every Codex task connects to the same process, so all clients share the camera registry, phone PIN, and phone HTTPS listener.
...
Clients that cannot connect to Streamable HTTP can still launch the server over STDIO. That compatibility mode is intended for one client; see [STDIO compatibility](stdio-compatibility.md) for its setup and limitations.
